...
// this test is prepared for debug.
/*
type Target struct {
	Target   string
	Protocol string
...
func TestScan(t *testing.T) {
	...
	for _, target := range targets {
		results, err := Scan(target.Target, target.Protocol, target.FromPort, target.ToPort)
		var findings []*finding.FindingForUpsert
		for _, r := range results {
			t.Logf("nmapResult: %v\n", r)
			findings = append(findings, r.GetFindings(1001, "dataSource", "hogehoge")...)
		}
		t.Logf("Error: %v\n", err)
